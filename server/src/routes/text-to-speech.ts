...
router.get('', async (req: Request, res: Response) => {
    ...
    const ssmlGender: SsmlVoiceGender = req.query.gender as unknown as SsmlVoiceGender;
    ...
    try {
        ...
        console.log(`Got Text-to-Speech response for text: ${text} and gender: ${ssmlGender}`);

        res.writeHead(StatusCodes.OK, {
            'Content-Type': 'audio/mp3'
        }).end(ttsResponse.audioContent);
    } catch (error) {
        const message = `Failed to fetch Text-to-Speech for text: ${text} and gender: ${ssmlGender}`;
